...
function Recieved({message}) {
	...
	let M_Time="";
	useEffect(()=>{
		let Sender = message.Sender_Id ? message.Sender_Id : message.Sender_ID;
		message && Sender && firestore.collection("Users").where("UID","==",Sender).get().then((Snap)=>{
			set_Sender(Snap.docs[0].data());
		})
	},[message])
							...
							<div className="group relative">
								...
								</div>
								{
									...
									<>
										...
										<div className=" mt-5 min-w-[170px] max-w-[320px] pr-5 flex justify-between w-full text-xs font-normal text-gray-500 dark:text-gray-400 gap-2">
											{
												formatBytes(message.Message.Size)
											}
											<div className="flex">
									...
									<>
										...
										<div className=" mt-5 min-w-[170px] max-w-[320px]  pr-6 flex justify-between w-full text-xs font-normal text-gray-500 dark:text-gray-400 gap-2">
											{
